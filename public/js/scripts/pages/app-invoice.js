...
$(function () {
  ...
  $(document).on('click', '.add-new-phone', function () {
    ...
    select2.on('change', function () {
      var $this = $(this),
        renderDetails =
          ...
          '</p>' +
          '</div>';
      $('.row-bill-to').find('.phone-details').remove();
      $('.row-bill-to').find('.col-bill-to').append(renderDetails);
    });

    select2.on('select2:open', function () {
      if (!$(document).find('.add-new-phone').length) {
        $(document)
          .find('.select2-results__options')
          .before(
            '<div class="add-new-phone btn btn-flat-success cursor-pointer rounded-0 text-left mb-50 p-50 w-100" data-toggle="modal" data-target="#add-new-phone-sidebar">' +
              feather.icons['plus'].toSvg({ class: 'font-medium-1 mr-50' }) +
              '<span class="align-middle">Add New Phone</span></div>'
          );
      }
    });
  }
